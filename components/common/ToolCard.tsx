  ...
  if (variant === "featured") {
    return (
      ...
      >
        <Link
          href={`/tool/${tool.slug}`}
          className="group block glass-card rounded-3xl p-6 lg:p-7 overflow-hidden relative card-3d"
        >
          ...
          <div className="relative">
            <div className="flex items-start justify-between mb-5">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary-500/10 to-purple-500/10 flex items-center justify-center ring-1 ring-primary-200/50 dark:ring-primary-800/50 group-hover:scale-110 transition-transform duration-300">
                <span className="text-xl font-bold text-primary-600 dark:text-primary-400">{tool.logo}</span>
              </div>
              <div className="flex items-center gap-2">
                <Badge variant={tool.trending ? "warning" : "info"}>{tool.category}</Badge>
                {tool.featured && <Award className="w-5 h-5 text-amber-400" />}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-5">
              {tool.features.slice(0, 3).map((f) => (
                <span key={f} className="px-2.5 py-1 text-xs rounded-lg bg-primary-50 dark:bg-primary-950/30 text-primary-700 dark:text-primary-300 font-medium">
                  {f}
                </span>
              ))}
            </div>
