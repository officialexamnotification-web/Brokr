...
const STOCK_SYMBOLS = [
  ...
  "NFLX", "AMD", "CRM", "ADBE", "QCOM", "INTC", "CSCO", "IBM", "UBER", "DIS", "KO", "PEP", "MCD", "NKE", "BA", "CAT", "GE", "UNH", "MRK", "PFE", "CVX", "TMO", "AMGN", "GS", "MS", "LIN", "RTX", "LOW", "SBUX", "PLTR",
];
const FOREX_PAIRS = [
...
const titles: Record<MarketKind, { title: string; description: string; source: string }> = {
  crypto: { title: "Cryptocurrency Market", description: "Top 50 cryptocurrencies with price, performance, liquidity, and market-cap details.", source: "CoinGecko" },
  forex: { title: "Forex Market", description: "44 major and cross-currency reference rates across US, Canadian, UK, European, Australian, Swiss, Japanese, Singaporean, and other high-liquidity markets.", source: "Frankfurter / ECB" },
  stocks: { title: "US Stocks Market", description: "50 widely followed US-listed large-cap, technology, financial, healthcare, consumer, industrial, and energy stocks.", source: "StockData.org / Finnhub fallback" },
};
...
export default function MarketDataPage({ market }: { market: MarketKind }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        if (market === "crypto") {
          const cryptoData = await fetchCryptoMarketData(CRYPTO_IDS);
  ...
  const cryptoRows = useMemo(() => Object.entries(crypto).filter(([id, item]) => {
    const text = `${id} ${id.replaceAll("-", " ")}`.toLowerCase();
    return text.includes(query.trim().toLowerCase()) && typeof item?.usd === "number";
  }), [crypto, query]);
  const stockRows = useMemo(() => Object.entries(stocks).filter(([symbol, item]) => `${symbol} ${item?.name || ""}`.toLowerCase().includes(query.trim().toLowerCase())), [stocks, query]);
  ...
  const cryptoOffline = market === "crypto" && Object.values(crypto).some((item) => item.source === "offline");
  const forexOffline = market === "forex" && forex.source === "offline";
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-slate-500 dark:text-slate-400">{info.description} Prices may be delayed and are for informational purposes only.</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          ...
          <div className="glass-card rounded-2xl p-4"><p className="text-xs text-slate-500 dark:text-slate-400">Data source</p><p className="mt-1 text-sm font-bold text-slate-900 dark:text-white">{cryptoOffline || forexOffline ? "Offline reference snapshot" : info.source}</p></div>
        ...
        {!loading && error && <div className="glass-card rounded-2xl p-8 text-center text-amber-700 dark:text-amber-400">{error}</div>}
